---
layout: default
title: Nihiniverse - About
---
<div class="home-hero">
<div class="home-about section-container">
    <div class="home-max-width max-content-container">
        <div class="home-wrapper">
            <div class="home-text-container">
                <span class="Heading-text1">About Us</span>
                <h2 class="home-text2">
                  <span>Advancing Knowledge and </span>
                  <span>Innovation from Heart</span>
                </h2>
                <span class="home-text3">
                  <span>Our organization operates as a distinguished research company based in Hong Kong, a pivotal center for innovation and scholarly collaboration.</span>
                </span>
                <div class="home-checklist">
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Innovative Research Development</span>
                    </div>
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Knowledge Dissemination</span>
                    </div>
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Interdisciplinary Collaboration</span>
                    </div>
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Research Excellence</span>
                    </div>
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Impactful Solution</span>
                    </div>
                    <div class="home-check-item">
                        <img alt="image" src="assets/images/tick.png" class="home-image-tick"/>
                        <span class="home-text4">Sustainability and Growth</span>
                    </div>
                </div>
                <a href="https://www.facebook.com/Nihiniverse/" target="_blank" rel="noopener noreferrer">
                <img alt="Facebook" src="{{ '/assets/images/social_media/facebook.png' | relative_url }}"  class="home-image-social"/></a>
            </div>
        <div class="home-image-container">
            <img
                alt="image"
                src="assets/images/Main1.jpg"
                class="home-image"
                width="460" height="512"
            />
        </div>
        </div>
    </div>
</div>

